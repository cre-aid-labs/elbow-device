...
void LAController::controlLoop() {
  while(true) {
    ...
    unsigned long delta_micros = cur_enc_micros - prev_enc_micros;
    if((LACTRL_DELTA_THRESHOLD > delta_micros) && (delta_micros > 0)) {
      omega = (enc_pulses - prev_enc_pulses) / delta_micros;
